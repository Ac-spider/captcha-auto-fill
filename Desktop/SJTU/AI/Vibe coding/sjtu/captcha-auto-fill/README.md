...
自动检测网页中的验证码图片，使用 OCR 技术识别验证码内容，并自动填写到输入框中。专为上海交通大学 jaccount 登录页面优化，同时支持各类网站的验证码识别。
...
1. 点击扩展图标
2. 勾选"一键登录（自动提交，失败自动重试）"
3. 输入用户名密码后，扩展会自动完成验证码识别和登录
4. 登录失败会自动重试（最多 3 次）
...
A: 一键登录功能默认关闭，需要手动开启。扩展不会保存您的密码，只自动处理验证码部分。
